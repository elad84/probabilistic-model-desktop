...
export default class EditNetwork extends Component<Props> {
  ...
  handleNodeChosen(node) {
    ...
    const currentList = graphNodes.filter(graphNode => {
      ...
      return graphNode.id !== chosenNodeId && parents.indexOf(chosenNodeId) === -1 && reachableNodes.indexOf(graphNode.id) === -1;
    });
  ...
  setNodeDetails() {
    const {chosenNodeDomain, chosenNodeLabel, chosenNodeParents, chosenNode} = this.state;
    // set node range
  ...
  render() {
    const {handleAddNode, networkType, addControlNode, networkName, graph, unsaved} = this.props;
    const {chosenNodePotentialParents, dialogOpen, chosenNodeDomain, chosenNode, chosenNodeLabel, chosenNodeParents, nodeLabelExists} = this.state;

    const graphDetails = graph.graph;

    const controlNodeButton = networkType === 'ITERATIVE' ? <RaisedButton label="Add Control Node" style={addControlButtonStyle}
                                                                          onClick={() => addControlNode()}/> : null;
    ...
    return (
      <div>
        ...
        <div>
          ...
          <RaisedButton label="Add New Node" style={addNodeStyle} onClick={() => handleAddNode()}/>
          {controlNodeButton}
          <RaisedButton label="Save Network" style={addControlButtonStyle}
                        onClick={() => this.save()/* saveGraph(this.refs.fileName.getValue()) */}/>
          <RaisedButton label="Save As" style={addControlButtonStyle}
                        onClick={() => this.saveAs()/* saveGraph(this.refs.fileName.getValue()) */}/>
        </div>


        <div style={graphStyle}><GraphComponent handleNode={this.handleNodeChosen.bind(this)}/></div>
        <NodeDialog closeCallback={this.closeDialog.bind(this)}
                    open={dialogOpen}
