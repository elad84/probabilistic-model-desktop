...
export default class AjaxUtil{
    ...
    static postData(url, data, headers, responseCallback){
        let xhr = this.createCors("POST", url);
        if(!xhr){
        ...
        if(headers){
            for(let header in headers){
                headers.hasOwnProperty(header) && xhr.setRequestHeader(header, headers[header]);
            }
        }
        ...
        xhr.error = function(){
            console.log("error happened in the request");
        }
